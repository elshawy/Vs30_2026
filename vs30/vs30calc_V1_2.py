...
def run_vs30calc(
    p_paths: params_3.PathsParams,
    p_sites: params_3.SitesParams,
    p_grid: params_3.GridParams | None,
    p_ll: params_3.LLFileParams | None,
    p_geol: params_3.GeologyParams,
    p_terr: params_3.TerrainParams,
    p_comb: params_3.CombinationParams,
    n_procs: int,
):
    """
    ...
    measured_sites = sites_load_NZGD.load_vs(source=p_sites.source)
    measured_sites_points = np.column_stack(
        (measured_sites.easting.values, measured_sites.northing.values)
    )

    MODEL_MAPPING = {"geology": model_geology_new, "terrain": model_terrain}
    ...
    for model_setup in [p_geol, p_terr]:
        ...
        if model_setup.update == "prior":
            ...
            if p_sites.source == "cpt":
                ...
                model_table = model_fixed.cluster_update(
                    model_table, measured_sites, model_setup.letter
                )
            else:
                model_table = model_fixed.posterior(
    ...
    if p_geol.update != "off" and p_terr.update != "off":
        ...
        if p_ll is not None:
            for prefix in ["", "mvn_"]:
                table[f"{prefix}vs30"], table[f"{prefix}stdv"] = model_fixed.combine_models(
                    p_comb,
            ...
            model_fixed.combine_tiff(p_paths.out, "combined.tif", p_grid, p_comb, *tiffs)
            model_fixed.combine_tiff(
    ...
    if p_ll is not None:
        ...
        copyfile(
            os.path.join(sites_load_NZGD.data, "qgis_points.qgz"),
            os.path.join(p_paths.out, "qgis.qgz"),
        )
    else:
        copyfile(
            os.path.join(sites_load_NZGD.data, "qgis_rasters.qgz"),
            os.path.join(p_paths.out, "qgis.qgz"),
        )
