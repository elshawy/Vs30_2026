...
def combine_models(opts, vs30a, stdva, vs30b, stdvb):
    ...
    - If one model's value is NaN, use the other model's value.
    - If both models are NaN, the result remains NaN.
    """

    nan_a = np.isnan(vs30a)
    nan_b = np.isnan(vs30b)


    both_nan = nan_a & nan_b
    combined_vs30 = np.full_like(vs30a, np.nan)
    combined_stdv = np.full_like(stdva, np.nan)
    if np.any(both_nan):
        combined_vs30[both_nan] = np.nan
        combined_stdv[both_nan] = np.nan


    a_only_valid = ~nan_a & nan_b
    b_only_valid = nan_a & ~nan_b
    combined_vs30[a_only_valid] = vs30a[a_only_valid]
    combined_stdv[a_only_valid] = stdva[a_only_valid]
    combined_vs30[b_only_valid] = vs30b[b_only_valid]
    combined_stdv[b_only_valid] = stdvb[b_only_valid]
    ...
    if np.any(both_valid):
        if opts.stdv_weight:
            ...
            m_a = (stdva[both_valid]**2) ** -opts.k
            m_b = (stdvb[both_valid]**2) ** -opts.k
            w_a = m_a / (m_a + m_b)
            w_b = m_b / (m_a + m_b)
        else:
        ...
        log_ab = (
            np.log(vs30a[both_valid]) * w_a
            + np.log(vs30b[both_valid]) * w_b
        )
        combined_vs30[both_valid] = np.exp(log_ab)
        combined_stdv[both_valid] = np.sqrt(
            w_a * ((np.log(vs30a[both_valid]) - log_ab) ** 2 + stdva[both_valid]**2)
            + w_b * ((np.log(vs30b[both_valid]) - log_ab) ** 2 + stdvb[both_valid]**2)
        )
...
def _new_mean(mu_0, n0, var, y, n):
    return exp((n0 / var * log(mu_0) + log(y)*n / var) / (n0 / var + n / var))


def _new_var(sigma_0, n0, uncertainty, mu_0, y, n):
    mean_shift = (n0*n / (n0 + n))* (log(y) - log(mu_0))**2
    return (n0 * sigma_0 * sigma_0 + uncertainty * uncertainty + mean_shift) / (n0 + n)
...
def posterior(model, sites, idcol, n_prior=3, min_sigma=0.5):
    ...
    for _, r in sites.iterrows():
        ...
        var = _new_var(stdv[m], n0[m], r.uncertainty, vs30[m], r.vs30, n)
        vs30[m] = _new_mean(vs30[m], n0[m], var, r.vs30, n)
        stdv[m] = sqrt(var)
