...
data = os.path.join(os.path.dirname(__file__), "data/updated")
#DATA_CPT = os.path.join(data, "cptvs30_v25.ssv")
DATA_KAISERETAL = os.path.join(data, "Geonet Metadata Summary_v1.4_NOVS30MAP.csv")
DATA_MCGANN = os.path.join(data, "McGann_cptVs30data.csv")
DATA_WOTHERSPOON = os.path.join(data, "Measured_Vs30data_NZGD.csv")
...
#    """#
 ...
 #   mcgann = pd.read_csv(
 #       DATA_MCGANN,
  #      usecols=[3, 4, 7] if downsample else [0, 1, 2, 3],
 #       names=["easting", "northing", "vs30","uncertainty"],
#        skiprows=1,
#        engine="c",
#        dtype=np.float32,
#    )
...
def load_wotherspoon_vs():
    ...
    wotherspoon = pd.read_csv(
        ...
        converters={
            "q": lambda text: int(text),
            "station": lambda s: s.strip()
        }
    )

    # Transform coordinates (ensure this is correct for your data!)
    wotherspoon["easting"], wotherspoon["northing"] = wgs2nztm.transform(
        wotherspoon["easting"].values,
        wotherspoon["northing"].values
    )
